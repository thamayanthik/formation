package com.formation.algomarthon.recursion;

/*
A digit string is good if the digits (0-indexed) at even indices are even and
the digits at odd indices are prime (2, 3, 5, or 7).

For example, "2582" is good because the digits (2 and 8) at even positions are even and
the digits (5 and 2) at odd positions are prime. However, "3245" is not good because 3 is at an even index
but is not even.

Given an integer n, return the total number of good digit strings of length n.
Since the answer may be large, return it modulo 109 + 7.

A digit string is a string consisting of digits 0 through 9 that may contain leading zeros.

Example 1:
Input: n = 1
Output: 5

Explanation: The good numbers of length 1 are "0", "2", "4", "6", "8".

Example 2:
Input: n = 4
Output: 400

Example 3:
Input: n = 50
Output: 564908303

 */
public class CountGoodNumbers {
    private static int mod = 1_000_000_007;

    public static void main(String[] args) {
        System.out.println(countGoodNumbers(10));
    }

    public static long calculateXPowerY(long a, long b) {
        System.out.println("a : " + a + ", b : " + b);
        if (b == 0) {
            return 1;
        }
        if (b == 1) {
            return a;
        }
        long temp = calculateXPowerY(a, b / 2) % mod;
        System.out.println("a : " + a + ", b : " + b + ", tmp : " + temp);
        if (b % 2 == 0) {
            temp = temp * temp;
        } else {
            temp = ((a) * (temp * temp));
        }

        System.out.println("returns : " + temp);
        return temp;
    }

    public static int countGoodNumbers(long n) {
        long oddCount = n / 2;
        long evenCount = n - oddCount;
        System.out.println("OddCount : " + oddCount + ", EvenCount : " + evenCount);
        long evenPos = calculateXPowerY(5, evenCount) % mod;
        long oddPos = calculateXPowerY(4, oddCount) % mod;
        System.out.println("evenPos : " + evenPos + ", oddPos : " + oddPos);
        long total  = evenPos * oddPos;
        System.out.println("Total : " + total);

        int ans = (int) (total % mod);
        return ans;
    }

}
